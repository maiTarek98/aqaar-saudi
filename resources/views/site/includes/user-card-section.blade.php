                                ...
                                <div class="user-img">
                                    @if($property->access_links->where('source_user_id', auth('web')->user()->id)->isNotEmpty())
                                        @php $link = $property->access_links->where('source_user_id', auth('web')->user()->id)->first(); 
                                                $url2 = url('verify-property/'.$link->token.'?source=external&ref='.$link->current_level);
                                                $qrCode_v = \SimpleSoftwareIO\QrCode\Facades\QrCode::size(200)->generate($url2);
                                        @endphp
                                    <div class="copy-text flex-column">
                                          @if(auth('web')->user()->id == $property->added_by) 
                                          <input type="text" class="text" value="{{$url2}}" style="opacity: 0;margin-bottom: -18px;">
                                          @endif
                                          <button>
                                            <div class="user-img">
                                                {!! $qrCode_v !!}
                        ...
                        <div class="row align-items-center justify-content-between py-3 gy-2">
                            <div class="col-12">
                                <h6 class="user-name text-center">
                                    <b>رقم العرض {{$property->listing_number}}</b>
                                    @if($link)
                                    <b style="float:inline-end;">{{$link->current_level}}</b>
                                    @endif
                                </h6>
                            </div>
                            <div class="col-6 col-md-4">
